...
const TestimonialSection: FC = () => {
  ...
  // Periksa apakah tombol navigasi perlu ditampilkan
  useEffect(() => {
    const checkScroll = () => {
      if (containerRef.current) {
        setCanScrollLeft(containerRef.current.scrollLeft > 0);
        setCanScrollRight(containerRef.current.scrollLeft + containerRef.current.clientWidth < containerRef.current.scrollWidth);
      }
    };
    checkScroll();
  ...
  return (
    <section className="py-16 px-6 max-w-7xl mx-auto text-center">
      ...
      <div className="relative flex items-center justify-center" onMouseEnter={() => setIsHovered(true)} onMouseLeave={() => setIsHovered(false)}>
        ...
        {/* Container Testimoni dengan Snap Scroll */}
        <div ref={containerRef} className="flex overflow-x-auto space-x-4 w-full scrollbar-hide snap-x snap-mandatory">
          {defaultTestimonials.map((testimonial, index) => (
            <div key={index} className="min-w-[300px] md:min-w-[350px] bg-white dark:bg-gray-900 shadow-lg rounded-xl p-6 text-center flex-shrink-0 border border-gray-200 dark:border-gray-700 snap-center">
